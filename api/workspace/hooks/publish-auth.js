...
const generateEmail = ({email, token}) => {
  ...
  return {
    ...
    text: `Click here to reset your password: ${config.get('publish.host')}:${config.get('publish.port')}/sign-in/${token}`,
    html: `Click here to reset your password: ${config.get('publish.host')}:${config.get('publish.port')}/sign-in/${token}`
  }
}
